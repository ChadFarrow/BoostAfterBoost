...
export class IRCClient {
  ...
  async connect() {
    ...
    this.client = new irc.Client(server, this.config.nickname, {
      ...
      autoConnect: true,
      channels: this.config.channels,
      realName: this.config.realName || 'BoostAfterBoost Reader Bot',
      ...
      // `node-icu-charset-detector` module on every message, which isn't
      // installed here and spams "Cannot find module" errors. ZeroNode is
      // UTF-8 and this bot is read-only, so default decoding is correct.
      showErrors: true,
      debug: process.env.IRC_DEBUG === 'true'  // off by default: debug logs raw PASS
    });
