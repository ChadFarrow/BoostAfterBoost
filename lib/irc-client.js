...
export class IRCClient {
  ...
  async connect() {
    ...
    // IMPORTANT: Always use ZNC bouncer connection
    // Server: localhost (ZNC bouncer)
    // Port: 6697 (SSL)
    // Password format: ircbots/zeronode:password
    const zncServer = 'localhost';
    const zncPort = 6697;
    const zncPassword = `ircbots/zeronode:${this.config.password || ''}`;

    // Ensure ZNC is running before attempting connection
    logger.info('Ensuring ZNC is running before connection...');
    const zncHealthy = await this.ensureZncRunning();
    if (!zncHealthy) {
      logger.error('Cannot connect: ZNC is not running and failed to start');
      return;
    }

    logger.info('Connecting to ZNC bouncer...', {
      server: zncServer,
      port: zncPort,
      channels: this.config.channels,
      passwordFormat: 'ircbots/zeronode:***'
    });

    this.client = new irc.Client(zncServer, this.config.nickname, {
      port: zncPort,
      secure: true,
  ...
  setupEventHandlers() {
    this.client.on('registered', () => {
      logger.info('Successfully connected to ZNC bouncer');
      this.isConnected = true;
      this.reconnectAttempts = 0;
      this.startKeepAlive();
      this.startZncHealthCheck();
    });
